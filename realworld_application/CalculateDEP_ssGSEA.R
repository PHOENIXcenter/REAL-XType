...
f_ssGSEA_heatmap <- function(label, pro_matrix, output_name)
{
  # 1 DEP calculation ####
  library(limma)
  library(dplyr)
...
#  min_expression_cutoff <- 0.75
  ...
  #pro_matrix <- normalizeQuantiles(pro_matrix, ties = TRUE)
  ...
  #         sort = FALSE)
  # allpro <-
  #   allpro[!is.na(allpro$gene.symbol), ] #remove the proteins without gene symbols
  ...
  save(DEP_S1_up,
       ...
       DEP_S3_up,
       DEP_S3_down,
       file = paste0(output_name,"_subtype_DEP.RData")
  )
  ...
  # ssgsea_score_mean2 <- ssgsea_score_select[!duplicated(ssgsea_score_select$name),]
  ...
  # png(paste0(output_name,'_ssgsea_subtype_mean_neutrophil_immune.png'),width=1500*3,height=1200*3,res=200)
  pdf(paste0(output_name,'_ssgsea_subtype_mean_neutrophil_immune.pdf'))
  h <- h3 + h2
  print(h)
  dev.off()
}

data_path <- file.path("D:", "Data", "Tumor_clinic")
#load(file.path(data_path, "HCC-III-center1-401TP_center2-299TP_center3-324T_quantile_20210206.RData"))
#load(file.path(data_path, "ClinInfo_Center123_20210105.RData"))
load(file.path(data_path, "HCC_raw_data.RData"))
pro_matrix_list <- list(Jiang_data, SH_data, GZ_data, FZ_data, Gao_data)

# output_path <- file.path(data_path, "realword", "xtype-LOSO-5FCV", "para-34","seed-0", "km_plots")
output_path <- file.path(data_path, "random_forest")
setwd(output_path)
center_names = c("Jiang","SH", "GZ", "FZ", "Gao")

library(plyr)
for(center_id in 5:5){
  label_center <- read.csv(
    # paste0("./", center_names[center_id], "_0AB_preds.csv"),
    paste0("./", center_names[center_id], ".csv"),
    header = TRUE
  )
  labels <- label_center[, c("patients", "assignment")]
  ...
  f_ssGSEA_heatmap(
    label=labels, 
    pro_matrix=pro_matrix_list[[center_id]], 
    output_name=paste0(center_names[center_id], "_0AB_ssGSEA"))
}
