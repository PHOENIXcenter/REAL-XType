...
@tf.function
def train_step(x1, y1, x2, t1, e1, t2, e2, y2, classifier, optimizer, loss_rates, droprate, alpha, use_entropy=True):
    with tf.GradientTape(persistent=True) as tape:
        logits = classifier(x1, droprate)
        feat = classifier.encode(x2, droprate)
        r = classifier.risk(feat)

        loss_su = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=y1, logits=logits))
        loss_nll = negative_log_likelihood(r, t1, e1) + negative_log_likelihood(r, t2, e2)
        ...
        if use_entropy:
            ...
            weight = tf.zeros_like(y2, dtype=tf.float32)
            for d_id in range(classifier.domain_num):
                d_weight = tf.where(y2 == d_id, entropy, 0.)
                d_weight = d_weight / tf.stop_gradient(tf.reduce_sum(d_weight) + 1e-10)
                weight += d_weight
            loss_da = tf.reduce_sum(weight * tf.nn.sparse_softmax_cross_entropy_with_logits(labels=y2, logits=logits_domain)
            ) / tf.stop_gradient(tf.reduce_sum(weight))
        else:
            loss_da = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=y2, logits=logits_domain))
